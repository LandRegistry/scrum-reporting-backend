...
def get_projects(project_id):
    ...
    for row in res:
        ...
        res2.append({'name': row.project_name, 'programme_id': row.programme_id, 'id': row.id, 'product_owner': row.product_owner, 'scrum_master': row.scrum_master, 'sprint_array': sprint_array})
    return Response(json.dumps(res2),  mimetype='application/json')
...
def update_sprint(sprint_id):
    sprint_request = request.get_json(force=True)
    res = sprints.query.filter(projects.id == sprint_id).order_by(sprints.start_date).all()
    if (len(res) == 1):
